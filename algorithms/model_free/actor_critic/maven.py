...
MAVEN addresses one of the biggest challenges in multi-agent reinforcement learning:
effective exploration. It uses latent variables and mutual information to encourage
diverse behaviors and better exploration strategies.

Key Innovation - Diversified Exploration:
MAVEN introduces latent variables (hidden factors) that encourage agents to explore
different behavioral strategies within each episode:
- Latent variables guide agent behavior throughout an episode
- Mutual information maximization encourages diverse strategies
- Different latent values lead to different exploration patterns

How MAVEN Works:
1. Sample latent variable z at episode start
2. Condition agent policies on both observation and z
3. Use mutual information to encourage diverse latent-action mappings
4. Value functions also conditioned on latent variables
5. Results in diverse, coordinated exploration strategies

When to Use MAVEN:
✅ Environments requiring sophisticated exploration
✅ Sparse reward environments where exploration is critical
✅ Complex coordination tasks
✅ When standard epsilon-greedy is insufficient
✅ Research settings focusing on exploration

Key Advantages:
✅ Significantly improved exploration over standard methods
✅ Encourages diverse, meaningful behavioral strategies  
✅ Theoretically grounded approach using mutual information
✅ Can discover multiple solutions to the same task
✅ Works well with existing value-based methods

Limitations:
❌ Additional complexity from latent variable management
❌ Requires careful tuning of mutual information terms
❌ Higher computational cost due to additional networks
❌ More complex to implement than standard algorithms

Comparison with Other Algorithms:
- vs QMIX: Adds sophisticated exploration to value-based learning
- vs MADDPG: Uses latent variables rather than noise for exploration
- vs Standard Methods: Much better exploration but more complex

For MARL Beginners:
MAVEN is quite advanced! Master QMIX and VDN first. MAVEN solves the problem:
"How do we get agents to explore effectively as a team?" It's like giving
each agent a different "personality" that guides their exploration strategy.

Paper: "MAVEN: Multi-Agent Variational Exploration" (2019)
Use Cases: Sparse reward environments, complex exploration tasks, discovering diverse strategies
"""
...
from torch.distributions import Normal, Categorical  # For probability distributions
...
class MAVENAgent(MARLAgent):
    """
    Multi-Agent Variational Exploration Agent.
    
    This agent uses latent variables to guide exploration and discovery of
    diverse behavioral strategies. Each episode, the agent samples a latent
    variable that influences its behavior throughout that episode.
    
    Key Components:
    1. Q-Network: Conditioned on both observation and latent variable
    2. Latent Variable: Sampled per episode to guide exploration
    3. Mutual Information: Encourages diverse latent-action mappings
    4. Target Networks: For stable temporal difference learning
    
    Architecture Insight:
    - Latent-Conditioned Q-Network: Q(obs, action | latent) → Q-values
    - Latent Space: Low-dimensional space of exploration strategies
    - MI Maximization: Ensures latent variables meaningfully affect behavior
    
    For MARL Beginners:
    Think of this as an agent with different "exploration personalities."
    At the start of each episode, it picks a personality (latent variable)
    that guides how it explores and behaves throughout that episode.
    """
    
    def __init__(self, agent_id: int, obs_space: Dict, action_space: int, config: Dict):
        """
        Initialize the MAVEN agent with latent variable exploration capabilities.
        ...
        Args:
            ...
            config (Dict): Configuration containing hyperparameters
                          Example: {'latent_dim': 8, 'noise_scale': 0.3, ...}
        
        For Beginners:
        This creates an agent that can learn multiple exploration strategies
        by using latent variables to guide its behavior.
        """
        ...
        # Epsilon-Greedy Exploration Parameters (still used alongside latent exploration)
        ...
        # MAVEN-Specific Latent Variable Parameters
        # These control the latent variable space and exploration diversity
        self.latent_dim = config.get('latent_dim', 8)       # Dimensionality of latent space
        self.noise_scale = config.get('noise_scale', 0.3)   # Scale of latent variable sampling
        
        # Latent-Conditioned Q-Network
        # This network takes both observations and latent variables as input
